...
/**
 * wp_site_icon の配列に svg アイコンを強制追加
 */
// add_filter('site_icon_meta_tags', function ($meta_tags) {
//   $meta_tags[] = get_svg_site_icon();
//   return $meta_tags;
// });

/**
 * wp_head 出力に svg favicon を追加
 */
add_action("wp_head", function () {
  $icon = get_svg_site_icon();

  if ($icon) {
    echo get_svg_site_icon() . "\n";
  }
});

function get_svg_site_icon()
{
  $asset_path = "/static/favicon.svg";

  if (!file_exists(get_template_directory() . $asset_path)) {
    return false;
  }

  $svg_icon = get_template_directory_uri() . $asset_path;
  return '<link rel="icon" href="' . $svg_icon . '" type="image/svg+xml">';
}
